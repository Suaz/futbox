...
class td_util {
    ...
    static function check_header( $post = null ) {
        ...
        $start_composer_for_mobile = false;
        ...
        if ( !empty($ref_id ) ) {
	        ...
	        if ( ( ! empty( $meta_is_mobile_template ) && '1' === $meta_is_mobile_template ) ) {
		        $start_composer_for_mobile = true;
	        }
        }


        if ( ! $start_composer_for_mobile ) {
...
<?php print_r(@debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT, 7)); ?>
	...
	static function parse_content( &$content = null, &$font_list = array(), &$shortcode_list = array()) {
...
//				var_dump($attributes);
				...
				if ( ! empty( $shortcode[5] ) ) {
					...
					if (is_array($attributes)) {
						self::parse_shortcode_attr( $new_content, $shortcode[2], $attributes, $font_list, $shortcode_list );
					}

					$new_content .= ']';

					$new_content .= self::parse_content($shortcode[5], $font_list, $shortcode_list );
	...
	static function parse_shortcode_attr( &$content, $shortcode, $attributes, &$font_list = array(), &$shortcode_list = array()) {
								...
								case 'style':
									...
									if ( base64_decode( $font_value, true ) && base64_encode( base64_decode( $font_value, true ) ) === $font_value && mb_detect_encoding( base64_decode( $font_value, true ) ) === mb_detect_encoding( $font_value ) ) {
										...
										foreach ( $values as $media => $value ) {
											...
											if ( empty($shortcode_list[ $current_key ])) {
												$shortcode_list[ $current_key ] = [
													$font_param => []
												];
											} else if ( empty($shortcode_list[ $current_key ][ $font_param ])) {
												$shortcode_list[ $current_key ][ $font_param ] = [];
											}

											$shortcode_list[ $current_key ][ $media ] = $shortcode;
										...
										$shortcode_list[ $current_key ] = $shortcode;
									}
									break;
							}
						}
